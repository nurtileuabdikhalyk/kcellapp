...
def filterView(request):
    qs = Product.objects.filter(price__range=(float(request.GET.get("min")), float(request.GET.get('max'))))

    paginator = Paginator(qs, 5)
    page_number = request.GET.get('page')
    pages = paginator.get_page(page_number)

    context = {'qs': pages, "categories": Category.objects.values("name"), }
    return render(request, 'cart/filter_product.html', context)
